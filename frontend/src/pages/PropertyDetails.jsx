...
export default function PropertyDetails() {
  ...
  const similarAds = useMemo(() => {
    if (!listing) return [];
    return allListings.filter((item) => item._id !== listing._id && isProperty(item)).slice(0, 5);
  }, [allListings, listing]);
  ...
  const nearbyAds = useMemo(() => {
    ...
    return allListings
      .filter((item) => item._id !== listing._id && isProperty(item) && String(item.location || '').toLowerCase().includes(currentLocation))
      .slice(0, 5);
  }, [allListings, listing]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F8FAFF] flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center text-sm text-[#6D7480]">Loading property details...</div>
        <Footer />
      </div>
    );
  }
  ...
  if (!listing) {
    return (
      <div className="min-h-screen bg-[#F8FAFF] flex flex-col">
        <Navbar isAuthenticated={!!localStorage.getItem('token')} />
        <div className="flex-1 flex items-center justify-center text-sm text-red-600">Property not found.</div>
        <Footer />
      </div>
    );
  }
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-[220px_minmax(0,1fr)_300px] gap-4 lg:gap-6 items-start">
          ...
          <section className="space-y-4">
              ...
              <div className="p-4 sm:p-5 border-t border-[#EEF0F3]">
                ...
                <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-2">
                  <div className="bg-[#F7F8FA] border border-[#E7E9ED] p-3">
                    <p className="text-[11px] text-[#9AA1AC] uppercase tracking-[0.14em] font-semibold">Type</p>
                    <p className="text-sm font-bold text-[#2B3036]">{listing.offerType || 'Sale'}</p>
                  </div>
                  <div className="bg-[#F7F8FA] border border-[#E7E9ED] p-3">
                    <p className="text-[11px] text-[#9AA1AC] uppercase tracking-[0.14em] font-semibold">Beds</p>
                    <p className="text-sm font-bold text-[#2B3036]">{property.beds || '-'}</p>
                  </div>
                  <div className="bg-[#F7F8FA] border border-[#E7E9ED] p-3">
                    <p className="text-[11px] text-[#9AA1AC] uppercase tracking-[0.14em] font-semibold">Baths</p>
                    <p className="text-sm font-bold text-[#2B3036]">{property.baths || '-'}</p>
                  </div>
                  <div className="bg-[#F7F8FA] border border-[#E7E9ED] p-3">
                    <p className="text-[11px] text-[#9AA1AC] uppercase tracking-[0.14em] font-semibold">Area</p>
                    <p className="text-sm font-bold text-[#2B3036]">{property.sqft ? `${Number(property.sqft).toLocaleString()} sqft` : '-'}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-[#E7E9ED] rounded-sm p-5">
              ...
              <h3 className="text-lg font-bold text-[#2B3036] mt-6 mb-3">Property Features</h3>
              <ul className="space-y-2 text-[#5E6672]">
                {(property.amenities?.length ? property.amenities : listing.features || []).map((feature, idx) => (
          ...
          <aside className="space-y-4">
            ...
            <div className="bg-white border border-[#E7E9ED] rounded-sm p-4">
              ...
              <div className="space-y-3">
                ...
                {nearbyAds.map((item) => (
                  <Link key={item._id} to={`/property/${item._id}`} className="flex gap-2 group">
        ...
        <section className="mt-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {similarAds.map((item) => (
              <Link key={item._id} to={`/property/${item._id}`} className="bg-white border border-[#E7E9ED] rounded-sm overflow-hidden hover:shadow-[0_8px_18px_rgba(0,0,0,0.08)] transition">
                <div className="h-36 bg-[#EBEEF2]">
                  <img src={resolveImageUrl(item.image)} alt={item.title} className="w-full h-full object-cover" />
                </div>
                <div className="p-3">
                  <p className="text-[11px] text-[#8A8F99]">Property</p>
